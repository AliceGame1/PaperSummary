...
现在我们知道**D**里面两种单值格局都有了，因为**D**里的每个格局都是从ℂ里的某个格局经过一步e得到的，那么我们一定能在ℂ里找到两个格局C0,C1，使得D0=e(C0)是0-单值格局，而D1=e(C1)是1-单值格局，且c0经过一个event e'可以变成c1。不妨设是C0经过一步e’得到C1。（若则C1经过一步到C0下面的讨论也同样成立）
